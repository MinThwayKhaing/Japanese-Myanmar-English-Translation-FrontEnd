...
const FavouriteScreen = () => {
    ...
    const fetchFavorites = async (token, pageNumber) => {
        try {
            if (pageNumber === 1)
                setLoading(true);
            else
    ...
    const renderItem = ({ item }) => (<View style={styles.wordRow}>
      <TouchableOpacity style={styles.wordContainer} activeOpacity={0.7} onPress={() => navigation.navigate('UserWordDetail', { id: item.id })}>
    ...
    return (<SafeAreaView style={styles.safeContainer}>
      ...
      {loading && page === 1 ? (<ActivityIndicator size="large" color="#1A374D" style={{ marginTop: 30 }}/>) : favorites.length === 0 ? (<Text style={styles.emptyText}>You haven’t added any favorites yet.</Text>) : (<FlatList data={favorites} renderItem={renderItem} keyExtractor={(item) => item.id} onEndReached={loadMore} onEndReachedThreshold={0.5} ListFooterComponent={loadingMore ? <ActivityIndicator size="small" color="#1A374D"/> : null} ItemSeparatorComponent={() => <View style={styles.separator}/>} contentContainerStyle={{ paddingBottom: 40 }}/>)}
    </SafeAreaView>);
};
...
//# sourceMappingURL=FavouriteScreen.js.map