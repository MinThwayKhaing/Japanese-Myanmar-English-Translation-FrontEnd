...
import { View, Text, FlatList, TouchableOpacity, StyleSheet, ActivityIndicator, TextInput, StatusBar, Platform, Alert, } from 'react-native';
...
const ManageWordsScreen = () => {
    ...
    const fetchWords = async (pageNumber, search = '', authToken = token || '') => {
        try {
            if (pageNumber === 1)
                setLoading(true);
            else
                setLoadingMore(true);
            const data = await WordService.getAllWords(pageNumber, PAGE_LIMIT, search, authToken);
    ...
    const debouncedSearch = useCallback(debounce((text, authToken) => {
        setPage(1);
    ...
    const renderItem = ({ item }) => (<View style={styles.wordRow}>
    <View style={{ flex: 1 }}>
  ...
  <Text style={styles.translation}>Myanmar: {item.myanmar || '-'}</Text>
    ...
    {totalCount > 0 && (<Text style={{ marginHorizontal: 16, marginVertical: 10, fontSize: 16, fontWeight: '600', color: Colors.primary }}>
    Total Words: {totalCount}
  </Text>)}

      <View style={styles.searchContainer}>
        <Ionicons name="search" size={20} color="#777" style={{ marginRight: 6 }}/>
        <TextInput placeholder="Search words..." style={styles.searchInput} value={query} onChangeText={handleSearchChange}/>
      </View>

      <View style={{ flexDirection: 'row', marginHorizontal: 16, marginTop: 10 }}>
          <TouchableOpacity style={[styles.uploadButton, { marginRight: 10, backgroundColor: '#1A374D' }]} onPress={() => navigation.navigate('CreateWordScreen')}>
    <Text style={styles.uploadText}>Create Word</Text>
      ...
      {loading && page === 1 ? (<ActivityIndicator size="large" color={Colors.primary} style={{ marginTop: 30 }}/>) : words.length === 0 ? (<Text style={styles.emptyText}>No words found.</Text>) : (<FlatList data={words} renderItem={renderItem} keyExtractor={(item) => item._id || Math.random().toString()} onEndReached={loadMore} onEndReachedThreshold={0.5} ListFooterComponent={loadingMore ? <ActivityIndicator size="small" color={Colors.primary}/> : null} ItemSeparatorComponent={() => <View style={styles.separator}/>} contentContainerStyle={{ paddingBottom: 40 }}/>)}
    </SafeAreaView>);
};
...
//# sourceMappingURL=ManageWordsScreen.js.map